...
extension UIImage {
    ...
    public class func ppbaImage(name: String) -> UIImage? {
        ...
        var libResourceBundle: NSBundle?
        var path: String?
        
        let libResourceBundleURL = NSBundle.mainBundle().URLForResource("ZappSMGLibResources", withExtension: "bundle")
        
        if let libResourceBundleURL = libResourceBundleURL {
            libResourceBundle = NSBundle(URL: libResourceBundleURL)
            if let libResourceBundle = libResourceBundle {
                path = libResourceBundle.pathForResource(imageFinalName, ofType: "png")
            }
        }
        
        if libResourceBundle == nil {
            libResourceBundle = NSBundle(forClass: PBBAButton.self)
            if let libResourceBundle = libResourceBundle {
...
@objc public class PBBAButton: UIControl, Animatable {
    ...
    func didTapButton(button: PBBAButton) {
